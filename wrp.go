...
var (
	version  = "4.5.3 Alpha (unofficial)"
	srv      http.Server
	ctx      context.Context
...
func parseForm(w *wrpReq) {
	...
	w.quality, _ = strconv.ParseInt(w.req.FormValue("q"), 10, 64)
	if w.quality < 10 || w.quality > 100 {
		w.quality = defGeom.q
	}
	w.keys = w.req.FormValue("k")
	w.buttons = w.req.FormValue("Fn")
	w.imgType = w.req.FormValue("t")
	if w.imgType != "gif" && w.imgType != "jpeg" && w.imgType != "png" {
		w.imgType = defType
	}
...
func capture(w wrpReq) {
	...
	case "jpeg":
		...
		err = jpeg.Encode(&jpegbuf, i, &jpeg.Options{Quality: int(w.quality)})
		if err != nil {
			log.Printf("%s Failed to encode JPEG: %s\n", w.req.RemoteAddr, err)
			fmt.Fprintf(w.out, "<BR>Unable to encode JPEG:<BR>%s<BR>\n", err)
			return
		}
		img[imgpath] = jpegbuf
		ssize = fmt.Sprintf("%.0f KB", float32(len(jpegbuf.Bytes()))/1024.0)
		iw = i.Bounds().Max.X
		ih = i.Bounds().Max.Y
		log.Printf("%s Encoded JPEG image: %s, Size: %s, Quality: %d, %dx%d\n", w.req.RemoteAddr, imgpath, ssize, w.quality, iw, ih)	
	case "png":
...
func imgServer(out http.ResponseWriter, req *http.Request) {
	...
	case strings.HasPrefix(req.URL.Path, ".jpeg"):
...
func main() {
	...
	flag.StringVar(&defType, "t", "jpeg", "Image type: gif|jpeg|png")
	flag.StringVar(&fgeom, "g", "1280x960x100", "Geometry: width x height x quality, height can be 0 for unlimited")
	flag.StringVar(&tHTML, "ui", "wrp.html", "HTML template file for the UI")
	flag.Parse()
	if len(os.Getenv("PORT")) > 0 {
		addr = ":" + os.Getenv(("PORT"))
	}
	n, err := fmt.Sscanf(fgeom, "%dx%dx%d", &defGeom.w, &defGeom.h, &defGeom.q)
	if err != nil || n != 3 {
